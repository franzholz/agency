...
class tx_agency_marker {
	...
	public function replaceVariables (
		...
		$controlData = t3lib_div::getUserObj('&tx_agency_controldata');
	...
	if(confirm(\'' . $this->langObj->getLL('confirm_file_delete') . '\')) {
		...
		return false;
			...
			for ($i = sizeof($filenameArray); $i < $number + sizeof($filenameArray); $i++) {
				...
				$this->langObj->getLL('tooltip_' . (($cmd == 'invite') ? 'invitation_' : '')  . 'image') . '" size="40" type="file" ' .
				tx_div2007_alpha5::classParam_fh002(
					'uploader-view',
					'',
					$prefixId
